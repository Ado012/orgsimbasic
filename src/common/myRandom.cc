...
// Created      : 
// Revision     : $Id: myRandom.cc 574 2013-10-08 15:56:26Z henrik $
//
...
#undef FAC
	...
	long int ran3Randomize( void ) 
	{
		...
		gettimeofday(&tp, 0);
		//seed = 1024 * tp.tv_sec + (uint) (.001024 * tp.tv_usec)
...
BoostRandoms::BoostRandoms(){
  struct timeval tp; // { long tv_sec; long tv_usec; }
  gettimeofday(&tp, 0);
  generator = new generator_type(static_cast<unsigned int>(1024 * tp.tv_sec + 
							   static_cast<unsigned int>(.001024 * tp.tv_usec)));
  //generator = new generator_type(static_cast<unsigned int>(std::time(0)));
...
void BoostRandoms::CreateGamma(double k){	
  if(gamma_created) delete gamma;
  gamma = new gamma_type (*generator, boost::gamma_distribution< > (k));
  gamma_created = true;
}
...
void BoostRandoms::test()
{
  BoostRandoms a = BoostRandoms();
  ...
  cerr << "int Binoöial 25 tries p = 0.6 \n";
  int z6;
  ...
  cerr << "double Cauchy mean 0 sig 1\n";
  double z7;
  a.CreateCauchy(0,1);
  for(int i = 0; i < 10; ++i){
    z7 = a.Cauchy();
  ...
  cerr << "double Exponential lambda = 2\n";
  double z11;
  ...
  cerr << "double Normal mean 0  sig 1\n";
  double z12;
  ...
  cerr << "double lognormal mean 0 sig 1\n";
  double z13;
